...
class ReverseDiffusion(Model):
    ...
    def reverse_diffusion(self, initial_noise, diffusion_steps, optimize, batch_size=1):
        ...
        for step in range(diffusion_steps):
            ...
            diffusion_times = tf.ones((num_images, 1, 1, 1)) - step * step_size  # eagertensor
    ...
    def train_step(self, images):  # noisy image再加noise预训练denoise网络
        ...
        gradients = tape.gradient(noise_loss, self.network.trainable_weights)  # 以噪声误差训练模型
        # gradients = tape.gradient(image_loss, self.network.trainable_weights)  # 以重建损失训练模型
